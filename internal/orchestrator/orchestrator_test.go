...
func TestRuleSignalSource(t *testing.T) {
	now := time.Date(2024, 1, 11, 12, 30, 0, 0, time.UTC)
	source := NewRuleSignalSource(func() time.Time { return now })
	...
	tests := []struct {
		...
		{name: "buy", sentiment: decimal.NewFromFloat(0.5), want: domain.ActionBuy, wantLots: 1},
		{name: "sell", sentiment: decimal.NewFromFloat(-0.7), want: domain.ActionSell, wantLots: 1},
		{name: "hold", sentiment: decimal.NewFromFloat(0.05), want: domain.ActionHold, wantLots: 0},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			signal, err := source.Generate(context.Background(), domain.FeatureContext{
				Ticker:        "SBER",
				NewsSentiment: test.sentiment,
			})
			if err != nil {
				t.Fatalf("Generate() error = %v", err)
			}
			if signal.Action != test.want {
				t.Fatalf("action = %q, want %q", signal.Action, test.want)
			}
			if signal.TargetLots != test.wantLots {
				t.Fatalf("target lots = %d, want %d", signal.TargetLots, test.wantLots)
			}
			if signal.Ticker != "SBER" {
				t.Fatalf("ticker = %q, want SBER", signal.Ticker)
			}
			if !signal.GeneratedAt.Equal(now) {
				t.Fatalf("generated at = %v, want %v", signal.GeneratedAt, now)
			}
...
func TestRuleSignalSourceEmptyTicker(t *testing.T) {
	_, err := NewRuleSignalSource(nil).Generate(context.Background(), domain.FeatureContext{})
	if err == nil {
