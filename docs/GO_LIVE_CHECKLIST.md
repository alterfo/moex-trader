...
| 4 | Fat-finger check rejects an order price more than 2% from current best bid/ask | `internal/risk/gate.go` `fatFingerOK` calculates bid/ask deviation with `decimal.Decimal` | `internal/risk/gate_test.go`: `TestHardenedGateFatFingerBoundary` covers exactly 2.00% vs 2.01% above/below; `TestHardenedGateFatFingerRejectsNonPositiveOrderPrice` |
| 5 | Live executor uses UUID v4 `OrderID` and is idempotent | `internal/executor/live.go` generates `uuid.NewString`, parses and enforces `uuid.Version() == 4`, and caches sent order IDs | `internal/executor/live_test.go`: `TestLiveExecutorPlacesOrder`, `TestLiveExecutorDuplicateOrderIDIsIdempotent` |
| 6 | Kill switch trips on >3% drawdown and blocks new signals until reset | `internal/risk/gate.go` `exceedsDrawdown`, `triggerKillSwitch`, `TripKillSwitch`, `ResetKillSwitch`; `internal/storage/storage.go` persists `kill_switch` | `internal/risk/gate_test.go`: `TestHardenedGateKillSwitchOnDrawdown`, `TestHardenedGateDrawdownBoundary`, `TestHardenedGateManualKillSwitch`, `TestHardenedGatePersistsAndReadsKillSwitch`; `internal/orchestrator/orchestrator_test.go`: `TestOrchestratorKillSwitchBlocksNextCycle` |
...
- Phase 3: Tinkoff market-data client, AlgoPack enrichment, Redis Streams bus,
  Prometheus metrics, and Telegram alerting are implemented and tested.
- Phase 4: hardened risk gate, idempotent live executor, persisted kill switch plus
  Ollama timeout watchdog, and verifier agent are implemented and tested.
...
- Coverage for the go-live-critical packages is reasonable: `internal/risk` 86.7%,
  `internal/executor` 72.3%, `internal/llm` 89.5%.
