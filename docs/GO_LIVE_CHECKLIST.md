...
| 3 | ~~Prompt contains a strict-JSON instruction and returns exactly one JSON object~~ — moot: `internal/llm` was removed on 2026-09-16, signals no longer come from a prompted model | — | — |
...
| 7 | ~~Every Ollama call has a 10s watchdog and timeouts skip the ticker instead of blocking the loop~~ — moot: LLM/Ollama was removed from the codebase entirely on 2026-09-16, the live signal source is the ensemble model | — | — |
...
- Phase 2 (historical): Ollama client, strict prompt/few-shot examples, bounded retry
  with HOLD fallback, orchestration wiring, and `cmd/llmbench` were implemented and
  tested, then removed entirely on 2026-09-16 in favor of the ensemble model.
...
- Phase 4: hardened risk gate, idempotent live executor, persisted kill switch, and
  ...
  mode and Finam live mode are still refused until account snapshot, order
  cancellation, and broker-specific reconciliation are verified.
